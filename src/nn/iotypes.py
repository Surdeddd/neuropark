...
def type_of(path: str, types: Types) -> str:
    ...
    if not ext:
        raise NnError(
            Exit.BAD_IO,
            bi(
                f"input {path} has no extension, type cannot be determined",
                f"у входа {path} нет расширения — тип не определить",
            ),
        )
    ...
    raise NnError(
        Exit.BAD_IO,
        bi(
            f"extension .{ext} is not described in capabilities.json (types)",
            f"расширение .{ext} не описано в capabilities.json (types)",
        ),
    )
    ...
    for needed in cap.extra:
        if needed not in got:
            raise NnError(
                Exit.BAD_IO,
                bi(
                    f"capability {cap.name} needs an extra input of type {needed}"
                    f" (given: {got or 'nothing'}), use --extra",
...
def ext_for(type_name: str, types: Types) -> str:
    ...
    if not exts:
        raise NnError(
            Exit.BAD_IO,
            bi(
                f"type {type_name} is not described in capabilities.json (types)",
                f"тип {type_name} не описан в capabilities.json (types)",
            ),
        )
    return exts[0]
