...
Готовы фазы 1–5: ядро, стыковка, квоты, досье, оркестрация. Осталась фаза 6 — автоскан по расписанию под три ОС с дрейф-алёртами и упаковка в плагин Claude Code (после неё появится `/nn` и команда попадёт в PATH). Заглушки стоят и честно называют фазу: транспорты `ssh`/`http`, провайдеры с `adapter`.
